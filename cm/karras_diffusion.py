...
class KarrasDenoiser:
    ...
    def consistency_losses(
        ...
        if self.loss_norm == "l1":
            ...
            if x_start.shape[-1] < 256:
                distiller = th.cat([distiller, distiller[:,1,:,].unsqueeze(1)], dim=1)
                distiller_target = th.cat([distiller_target, distiller_target[:,1,:,].unsqueeze(1)], dim=1)
            ...
            loss = (
                ...
                (
                        self.lpips_loss(
                    ((distiller[:,0:3,:,:]+1)/2.0) ,
                    ((distiller_target[:,0:3,:,:]+1)/2.0)
                )
                # +
                #         self.lpips_loss(
                #     ( distiller[:, 3 ,: ,:].unsqueeze(1).repeat(1,3,1,1) + 1) / 2.0,
                #     (distiller_target[:,3,:,:].unsqueeze(1).repeat(1,3,1,1) + 1) / 2.0
                # )
                )
                * weights
            )
    ...
    def get_feats(self, model, x_t, sigmas, **model_kwargs):
        import torch.distributed as dist
        ...
        model.eval()
        with th.no_grad():
             feats = model(c_in * x_t, rescaled_t,feat_need =True,**model_kwargs)
        return feats
...
def karras_sample(
    ...
    dims = x_start.ndim
    x_T = x_start + noise * append_dims(sigmas[add_noise_t].to(x_start.device), dims)
    # x_T = x_start + noise * append_dims(sigmas[40].to(x_start.device), dims)
    sample_fn = {
        "heun": sample_heun,
        "dpm": sample_dpm,
    ...
    def denoiser(x_t, sigma):
        model_out, denoised = diffusion.denoise(model, x_t, sigma, **model_kwargs)
        # if clip_denoised:
        # denoised = denoised.clamp(-1, 1)
        # model_out = model_out.clamp(-1,-1)
        return model_out ,denoised

    def getfeats(x_t, sigma):

        feats = diffusion.get_feats(model, x_t, sigma, **model_kwargs)
        return feats
...
def sample_onestep(
    ...
    with th.no_grad():
        if isfeat:
            return getfeats(x, sigmas[remove_noise_t] * s_in)
        else:
            return distiller(x, sigmas[remove_noise_t] * s_in)
